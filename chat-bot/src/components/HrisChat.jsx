...
function HrisChat({ user, handleLogout }) {
  ...
  const formatLinks = (text, baseIndex) => {
    if (!text) return null;
    // Split the text by full markdown links, but keep the links in the array
    ...
    return parts.map((part, i) => {
      ...
      if (match) {
        // It's a markdown link, return an <a> tag
        ...
        // It's a regular chunk of text, so we process it for raw URLs
        return part.split(/(https?:\/\/[^\s]+)/g).map((subPart, j) => {
          // Check if this sub-part is a raw URL
           ...
           <div className="flex-1 overflow-y-auto px-2 space-y-1">
              ...
              {conversations.map((chat) => (
                  <div key={chat.id} className="relative group flex items-center" >
                    <button onClick={() => handleSelectChat(chat.id)} className={`w-full text-left px-4 py-3 text-sm rounded-lg ${currentTheme.hoverBtn} truncate transition-all flex items-center gap-3 opacity-80 group-hover:opacity-100`}>
                    ...
                    <button onClick={(e) => handleDelete(chat.id, e)} className="absolute right-2 p-1.5 rounded-full bg-red-500/10 text-red-500 opacity-0 group-hover:opacity-100 hover:bg-red-500/20 transition-all z-10">
